...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class DroneAI : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (Mathf.Pow(drone_pos[0] - my_path[next].pos[0], 2) + Mathf.Pow(drone_pos[2] - my_path[next].pos[2], 2) < 1)
            {
        ...
        private double[] calculateDesiredConfiguration()
        {
            ...
            Vector3 drone_next = my_path[next].pos - drone_pos;

            if (drone_pos + drone_next != my_path[next].pos)
            {
                drone_next = drone_pos - my_path[next].pos;
            }
